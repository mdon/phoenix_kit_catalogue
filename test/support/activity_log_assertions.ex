defmodule PhoenixKitCatalogue.ActivityLogAssertions do
  @moduledoc """
  Helpers for asserting activity log entries landed with the right
  action, module, and metadata shape.

  Imported into `PhoenixKitCatalogue.DataCase` and
  `PhoenixKitCatalogue.LiveCase` so every DB-backed test can reach
  them.

  Postgres returns UUIDs as raw 16-byte binaries; the helpers normalise
  to the string form so callers can pass `location.uuid` directly.
  """
  ...
  ## Example
      ...
      assert_activity_logged("catalogue.created",
        ...
        metadata_has: %{"name" => "Kitchen", "status" => "active"}
      )
  """
  @spec assert_activity_logged(String.t(), keyword()) :: map()
  ...
  @spec refute_activity_logged(String.t(), keyword()) :: :ok
