defmodule PhoenixKitCatalogue.Test.Repo.Migrations.CatalogueSetup do
  ...
  def up do
    ...
    create_if_not_exists table(:phoenix_kit_cat_manufacturer_suppliers, primary_key: false) do
      add :uuid, :binary_id, primary_key: true
      add :manufacturer_uuid, references(:phoenix_kit_cat_manufacturers, column: :uuid, type: :binary_id, on_delete: :delete_all), null: false
      add :supplier_uuid, references(:phoenix_kit_cat_suppliers, column: :uuid, type: :binary_id, on_delete: :delete_all), null: false
      timestamps()
    end

    create_if_not_exists unique_index(:phoenix_kit_cat_manufacturer_suppliers, [:manufacturer_uuid, :supplier_uuid])
    ...
    create_if_not_exists table(:phoenix_kit_cat_categories, primary_key: false) do
      ...
      add :status, :string, null: false, default: "active"
      add :catalogue_uuid, references(:phoenix_kit_cat_catalogues, column: :uuid, type: :binary_id, on_delete: :delete_all), null: false
      add :data, :map, default: %{}
      timestamps()
    end
    ...
    create_if_not_exists table(:phoenix_kit_cat_items, primary_key: false) do
      ...
      add :status, :string, null: false, default: "active"
      add :category_uuid, references(:phoenix_kit_cat_categories, column: :uuid, type: :binary_id, on_delete: :nilify_all)
      add :manufacturer_uuid, references(:phoenix_kit_cat_manufacturers, column: :uuid, type: :binary_id, on_delete: :nilify_all)
      add :data, :map, default: %{}
      timestamps()
    end
